...
export function generateInsights(
  ...
  if (totalRounds >= INSIGHT_THRESHOLDS.FOCUS_INSIGHT) {
    ...
    if (stdDev > 6) {
      insights.push({
        ...
        description: `Score spread is ${stdDev.toFixed(1)} strokes; big numbers are driving volatility`,
  ...
  if (recentRounds.length >= 3 && olderRounds.length >= 3) {
    ...
    if (olderStd > 0 && ((olderStd - recentStd) / olderStd) >= 0.1) {
      insights.push({
        ...
        description: `Score volatility improved by ${(100 * (olderStd - recentStd) / olderStd).toFixed(0)}% over recent rounds`,
  ...
  if (totalRounds >= 2 && insights.length === 0) {
    ...
    insights.push({
      ...
      actionable: 'Keep tracking advanced stats to unlock deeper insights',
      minimumRounds: 2,
      priority: 3,
      dismissible: true
    });
