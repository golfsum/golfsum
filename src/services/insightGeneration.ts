...
export function generateInsights(
    ...
    if (count >= 3) {
      ...
      if (diff >= 3) {
        insights.push({
          ...
          description: `Back 9 is ${diff.toFixed(1)} strokes higher than the front on average`,
        ...
        insights.push({
          ...
          confidence: InsightConfidence.MEDIUM,
          title: 'Slow Start',
          description: `Front 9 is ${Math.abs(diff).toFixed(1)} strokes higher than the back on average`,
  ...
  if (totalRounds >= 2 && insights.length === 0) {
    ...
    insights.push({
      ...
      description: `Averaging ${avgScore.toFixed(1)} over ${totalRounds} round${totalRounds !== 1 ? 's' : ''} (best: ${bestLabel}, high: ${worstLabel})`,
