...
const getRateLimitStore = () => {
  if (!globalThis.__gsAnalyticsStatsRateLimit) {
    globalThis.__gsAnalyticsStatsRateLimit = new Map();
  }
  return globalThis.__gsAnalyticsStatsRateLimit;
};
...
export default async function handler(req, res) {
  ...
  try {
    const { rows } = await sql`
      select
        coalesce(sum(pageviews), 0) as visits,
