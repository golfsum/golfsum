...
const MAX_HOME_COURSE_LENGTH = 160;
...
const getRateLimitStore = () => {
  if (!globalThis.__gsAdminHomeCourseRateLimit) {
    globalThis.__gsAdminHomeCourseRateLimit = new Map();
  }
  return globalThis.__gsAdminHomeCourseRateLimit;
};
...
async function patchDoc(path, updates, token, fieldPaths) {
...
export default async function handler(req, res) {
  ...
  const homeCourseName = String(body?.homeCourseName || "").trim();
  if (!uid || uid.length > MAX_UID_LENGTH) return res.status(400).json({ error: "invalid_uid" });
  if (homeCourseName.length > MAX_HOME_COURSE_LENGTH) return res.status(400).json({ error: "invalid_home_course" });
  ...
  if (!ok) return res.status(502).json({ error: "upstream_failure" });
