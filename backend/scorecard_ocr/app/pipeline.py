...
def parse_scorecard_rows(rows_with_text: List[List[Dict[str, object]]]) -> Dict[str, object]:
    ...
    for row_index, row in enumerate(rows_with_text):
        ...
        logging.getLogger("app.pipeline").debug(
            "row_stats row=%s label=%s raw_count=%s mapped_count=%s par_like=%s hcp_like=%s yardage_like=%s",
            row_index,
            label,
            len(raw_values),
            len(values_by_hole),
...
def _normalize_tee_name(value: object) -> str:
...
def parse_scorecard_image(image_bytes: bytes, debug: bool = False, mode: str = "course") -> Dict[str, object]:
    ...
    def _merge_tee_boxes(entries: List[object], allow_yardages: bool) -> None:
        for tee in entries:
            ...
            normalized_name = _normalize_tee_name(name)
            existing_name = next(
                (
                    key
                    for key in set(
                        list(yardage_by_tee.keys())
                        + list(rating_m.keys())
                        + list(slope_m.keys())
                        + list(rating_w.keys())
                        + list(slope_w.keys())
                    )
                    if _normalize_tee_name(key) == normalized_name
                ),
                None,
            )
            tee_key = existing_name or name

            if allow_yardages:
                yardages = _normalize_list(tee.get("yardages"))
                if tee_key in yardage_by_tee:
                    existing = yardage_by_tee[tee_key]
                    yardage_by_tee[tee_key] = [
                        incoming if incoming is not None else existing[idx]
                        for idx, incoming in enumerate(yardages)
                    ]
                else:
                    yardage_by_tee[tee_key] = yardages

            if tee.get("ratingMen") is not None:
                rating_m[tee_key] = tee.get("ratingMen")
            elif tee_key not in rating_m:
                rating_m[tee_key] = None

            if tee.get("slopeMen") is not None:
                slope_m[tee_key] = tee.get("slopeMen")
            elif tee_key not in slope_m:
                slope_m[tee_key] = None

            if tee.get("ratingWomen") is not None:
                rating_w[tee_key] = tee.get("ratingWomen")
            elif tee_key not in rating_w:
                rating_w[tee_key] = None

            if tee.get("slopeWomen") is not None:
                slope_w[tee_key] = tee.get("slopeWomen")
            elif tee_key not in slope_w:
                slope_w[tee_key] = None

    _merge_tee_boxes(tee_boxes, allow_yardages=True)

    # Fallback pass: in completed mode, run course-only extraction if rating/slope
    # wasn't found on the combined card scan (often printed near rules text).
    has_men_rating_or_slope = any(value is not None for value in rating_m.values()) or any(
        value is not None for value in slope_m.values()
    )
    if mode == "completed" and not has_men_rating_or_slope:
        course_fallback = parse_scorecard_with_gemini(image_bytes, mode="course")
        if isinstance(course_fallback, dict) and "error" not in course_fallback:
            fallback_tees = course_fallback.get("teeBoxes")
            if isinstance(fallback_tees, list):
                _merge_tee_boxes(fallback_tees, allow_yardages=False)
                if any(value is not None for value in rating_m.values()) or any(
                    value is not None for value in slope_m.values()
                ):
                    guard_flags.append("ratings_recovered_from_course_mode")
    ...
    if player_payload:
        ...
        score_entries = [
            (idx, hole.score)
            for idx, hole in enumerate(player_holes)
            if isinstance(hole.score, int) and hole.score > 0
        ]
        if len(score_entries) >= 6:
            score_values = [score for _, score in score_entries]
            all_scores_hcp_range = all(1 <= value <= 18 for value in score_values)
            if all_scores_hcp_range:
                def _match_ratio(candidate: List[Optional[int]]) -> float:
                    comparable = [
                        (idx, score)
                        for idx, score in score_entries
                        if idx < len(candidate) and isinstance(candidate[idx], int) and candidate[idx] is not None
                    ]
                    if len(comparable) < 6:
                        return 0.0
                    matches = sum(1 for idx, score in comparable if candidate[idx] == score)
                    return matches / len(comparable)

                men_ratio = _match_ratio(handicap_men)
                women_ratio = _match_ratio(handicap_women)
                if max(men_ratio, women_ratio) >= 0.66:
                    for hole in player_holes:
                        hole.score = None
                    guard_flags.append("player_scores_match_handicap")
